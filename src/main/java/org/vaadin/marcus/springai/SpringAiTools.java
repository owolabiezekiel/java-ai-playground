...
public class SpringAiTools {
    ...
    return request -> {
      try {
        return carRentalService.getBookingDetails(
            request.bookingNumber(), request.firstName(), request.lastName());
      } catch (Exception e) {
        logger.warn(
            "Booking details: {}", NestedExceptionUtils.getMostSpecificCause(e).getMessage());
  ...
  public Function<ChangeBookingDatesRequest, String> changeBooking() {
    return request -> {
      carRentalService.changeBooking(
          request.bookingNumber(),
          request.firstName(),
          request.lastName(),
          request.date(),
  ...
  public Function<CancelBookingRequest, String> cancelBooking() {
    return request -> {
      carRentalService.cancelBooking(
          request.bookingNumber(), request.firstName(), request.lastName());
      return "";
    };
  }
